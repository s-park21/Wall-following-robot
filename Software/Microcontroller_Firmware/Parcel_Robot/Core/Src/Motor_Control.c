...
void init() {
	HAL_TIM_Encoder_Start(Right_Wheel_Encoder_Tim, TIM_CHANNEL_ALL);
	HAL_TIM_Encoder_Start(Left_Wheel_Encoder_Tim, TIM_CHANNEL_ALL);
}
...
void get_wheel_velocity(ekf_t *ekf) {
	float dt = (Micros(Micros_Timer) - ekf->encoder_read_time_us) / 1.0E6;
	// Read left wheel encoder position
	ekf->wheel_ang_vel[0] = (Left_Wheel_Encoder_Tim->Instance->CNT - ekf->prev_encoder_value[0]) / dt;
	ekf->wheel_ang_vel[1] = (Right_Wheel_Encoder_Tim->Instance->CNT - ekf->prev_encoder_value[1]) / dt;
	ekf->encoder_read_time_us = Micros(Micros_Timer);
}

/*
 * wheel_speed_pid_update()
 * wheel_pid_t* pid: PID struct
 * ekf_t* ekf: Kalman Filter struct containing state values
 * float left_wheel_speed: The desired speed of the left wheel
 * float right_wheel_speed: The desired speed of the right wheel
 * returns: void
 */
void wheel_speed_pid_update(wheel_pid_t *pid, ekf_t *ekf, float left_wheel_speed, float right_wheel_speed) {
	float dt = (Micros(Micros_Timer) - pid->pid_update_time_us) / 1.0E6;

	get_wheel_velocity(ekf);
	float left_wheel_error = left_wheel_speed - ekf->wheel_ang_vel[0];
	float right_wheel_error = right_wheel_speed - ekf->wheel_ang_vel[1];

	float lw_derivative = (left_wheel_error - pid->prev_left_wheel_error) / dt;
	float rw_derivative = (right_wheel_error - pid->prev_right_wheel_error) / dt;

	pid->left_wheel_integral_error += left_wheel_error * dt;
	pid->right_wheel_integral_error += right_wheel_error * dt;

	float left_wheel_output = pid->kp * left_wheel_error + pid->ki * pid->left_wheel_integral_error + pid->kd * lw_derivative;
	float right_wheel_output = pid->kp * right_wheel_error + pid->ki * pid->right_wheel_integral_error + pid->kd * rw_derivative;

	pid->prev_left_wheel_error = left_wheel_error;
	pid->prev_right_wheel_error = right_wheel_error;

	// Constrain outputs to be between 0 and 100
	if (left_wheel_output > 100)
		left_wheel_output = 100;
	else if (left_wheel_output < 0)
		left_wheel_output = 0;

	if (right_wheel_output > 100)
		right_wheel_output = 100;
	else if (right_wheel_output < 0)
		right_wheel_output = 0;

	// Set left wheel PWM output
	uint16_t compareValue = (PWM_Timer->Init.Period * left_wheel_output) / 100;
	__HAL_TIM_SET_COMPARE(PWM_Timer, 1, compareValue);

	compareValue = (PWM_Timer->Init.Period * right_wheel_output) / 100;
	__HAL_TIM_SET_COMPARE(PWM_Timer, 2, compareValue);

	pid->pid_update_time_us = Micros(Micros_Timer);
}
